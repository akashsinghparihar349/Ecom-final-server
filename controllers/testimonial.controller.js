...
async function getRecord(req, res) {
  try {
    let data = await Testimonial.find().sort({ _id: -1 }).populate("user", ["name"])
      .populate("product", ["name"])
    res.send({
      result: "Done",
      data: data,
...
async function getSingleRecord(req, res) {
  try {
    let data = await Testimonial.findOne({ _id: req.params._id }).populate("user", ["name"])
      .populate("product", ["name"])
    if (data) {
      res.send({
        result: "Done",
...
async function deleteRecord(req, res) {
  try {
    ...
    let errorMessage = error.keyValue ? Object.fromEntries(Object.keys(error.keyValue).map(key => [key, `Testimonial with this name is Already Exist`])) : Object.fromEntries(Object.keys(error.errors).map(key => [key, error.errors[key].message]))
    res.status(Object.values(errorMessage).length !== 0 ? 400 : 500).send({
      result: "Fail",
      reason: Object.values(errorMessage).length !== 0 ? errorMessage : "Internal Server Error"
    })
